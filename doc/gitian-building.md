...
*Setup instructions for a Gitian build of Veda Core using a Debian VM or physical system.*
...
- [Building Veda Core](#building-veda-core)
...
Clone the git repositories for Veda Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/vedapay/veda
```
...
in the Veda Core repository under 'Fetch and build inputs' to install sources which require
...
To build Veda Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Veda Core repository.
...
```bash
    ...
    From https://github.com/vedapay/veda
...
and the veda git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../veda/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/veda.git
SIGPATH=/some/root/path/veda-detached-sigs.git

./bin/gbuild --url veda=${BTCPATH},signature=${SIGPATH} ../veda/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/veda-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/veda-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/veda-osx-build.assert
```
...
[veda/gitian.sigs](https://github.com/vedapay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
