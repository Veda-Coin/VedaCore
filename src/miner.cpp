...
//		    printf("BitcoinMiner: .........fvNodesEmpty: %d.............IsInitialBlockDownload(): %d...masternodeSync.IsSynced(): %d\n", fvNodesEmpty, IsInitialBlockDownload(), masternodeSync.IsSynced());
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("VedaMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("VedaMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
